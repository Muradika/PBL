...
function initializeDropzone(dropzoneElement, type) {
    ...
    dropzoneElement.addEventListener("click", (e) => {
        ...
        const isControl = e.target.closest('.btn-preview') || e.target.closest('.remove-file');
...
function previewDocument() {
    ...
    // For Word/Excel files, trigger download
        ...
        alert('Document will be downloaded. You can open it with compatible software to preview.');
    }
}
...
function validateFormAndSubmit(e) {
    ...
    if (!uploadedFiles.image && !uploadedFiles.document) {
        if (!confirm('No files uploaded. Continue creating announcement without files?')) {
            e.preventDefault();
            return false;
        }
    }
